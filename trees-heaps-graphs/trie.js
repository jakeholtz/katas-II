...
var Trie = function() {
  this.trie = {}
  this.affix = function(childValue, parent) {
    parent[childValue] = {"*": false}
  }
  this.defix = function(childValue, parent) {
    delete parent[childValue];
  }
  this.markWhole = function(childLetter, parent) {
    parent[childValue]["*"] = true;
  }
}
...
var Trie = function() {
  ...
  this.addWord = function(word) {
    var pointOfEntry = this.contents;
    for (var k = 0; k < word.length; k++) {
      if (!pointOfEntry[word[k]]) {
        pointOfEntry[word[k]] = {'*': false}
      }
      if (k === word.length - 1) {
        pointOfEntry[word[k]]['*'] = true;
      }
      pointOfEntry = pointOfEntry[word[k]];
    }
    return this.contents;
  }
  this.returnCount = function(word, count = 0) {
    var pointOfEntry = this.contents;
    for (var k = 0; k < word.length; k++) {
      count += pointOfEntry[word[k]]['*'];
      pointOfEntry = pointOfEntry[word[k]]
    }
    return count - 1;
  }
...
var array = ["be", "may", "mayberry", "ma", "mainstream", "berry", "mayn", "er", "am", "in"]
//         0       1       5          0       3           2        0      0     0     0

function consoleSmallerWords(arr) {
  var newTrie = new Trie();
  for (var i = 0; i < arr.length; i++) {
    newTrie.addWord(arr[i]);
  }
  debugger;
  for (i = 0; i < arr.length; i++) {
    console.log("'" + arr[i] + "' consists of " + newTrie.returnCount(arr[i]) + " smaller word(s)");
  }
}